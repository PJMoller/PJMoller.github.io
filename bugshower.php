...
if(isset($stmt)){
    ...
    while($result = $stmt->fetch(PDO::FETCH_ASSOC)){//We fetch every record one by one and print it
        ...
        echo "<tr><td>{$result["BugID"]}</td><td>{$result["Product"]}</td><td>{$result["Version"]}</td><td>{$result["Hardware"]}</td><td>{$result["OS"]}</td><td>{$result["Frequency"]}</td><td>{$result["Solution"]}<a href=bugeditor.php>Edit</a></td><tr></table><br>";
    }
